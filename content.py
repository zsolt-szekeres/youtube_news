...
def get_subfolders(relative_path):
    ...
    subfolders = [entry for entry in all_entries if os.path.isdir(os.path.join(absolute_path, entry))]    
...
def save_all(summary, transcription, info, fname, local_config=None, cloud=None):  
    ...
    """   
    folder = str.split(fname,'\\')[0]+'\\'+str.split(fname,'\\')[1]
    name = str.split(fname,'\\')[2]
    ...
    if local_config:
        ...
        with open(folder+'\\'+str(max_int+1)+'\\'+name+'_'+version+'.json', 'w', encoding='utf-8') as f:
            json.dump({k: config.params[k] for k in ['gpt','chunking']}, f, ensure_ascii=False, indent=4)
...
def get_transcript(folder):
    fname=folder+str.split(folder,'\\')[4]    
