...
llm4 = LChatOpenAI(model_name='gpt-4',temperature=0, openai_api_key=config.params['auth_codes']['OpenAI_API_key'])
...
def get_config(local_config=None):
    ...
    return (map_prompt, combine_prompt, simple_prompt, simple_model, \
            mapreduce_model, chunk_size, overlap)
    
def get_context_for_bot(query, text, local_config=None, context_num=3,context_len=1000, context_overlap=50):

    map_prompt, combine_prompt, simple_prompt, simple_model, \
        mapreduce_model, chunk_size, overlap = get_config(local_config)
...
def get_summary(text, ntokens, local_config=None):
    ...
    if mapreduce_model=='gpt-3.5-turbo':            
        ...
        summary_chain = load_summarize_chain(llm=llm4,
                                        chain_type='map_reduce',
                                        map_prompt=map_prompt_template,
                                        combine_prompt=combine_prompt_template,
                                          #verbose=True
