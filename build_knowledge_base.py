...
if __name__ == '__main__': 
    ...
    # Add arguments
    parser.add_argument("-p", "--prev_inv", type=str, default=" ", help="Previous inventory file.")
    parser.add_argument("-n", "--new_inv", type=str, default=r'inventory.xlsx', help="New inventory file.")
    parser.add_argument("-s", "--chunk_size", type=int, default=10000, help="Chunk size in tokens.")
    parser.add_argument("-o", "--chunk_overlap", type=int, default=1000, help="Overlap of chunks in tokens.")
    ...
    text_splitter_search = RecursiveCharacterTextSplitter(chunk_size=args.chunk_size, \
    ...
    for f in tqdm(folders):
            transcript_file = os.path.join('videos',os.path.join(f,f+'_transcript.json'))            
            ...
            if (os.path.exists(transcript_file) & os.path.exists(info_file)):
                ...
                try:               
                    if info['id'] not in loaded:     
                        ...
                        if ((info['channel']=='Andrew Huberman') or (info['channel']=='Lex Fridman')):
                            guest = title.split(":")[0]
                            if guest.count(' ') < 3:
                            ...
                            try:
                                ...
                                for d in docs_search:
                                    d.metadata = {'host':info['channel'], 'guest':guest, 'upload_date':info['upload_date'], 'id':info['id']}
                                persist_directory = base_directory+'/'+info['channel']+'_'+str(args.chunk_size)+'_'+str(args.chunk_overlap)
                                db = Chroma.from_documents(docs_search, OpenAIEmbeddings(),persist_directory=persist_directory)
